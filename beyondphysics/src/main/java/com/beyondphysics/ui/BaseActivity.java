...
public class BaseActivity extends AppCompatActivity {
    ...
    public void dismissAndRemoveAllProgressDialog() {
        ...
        while (iterator.hasNext()) {
            ...
            if (progressDialog != null) {//add方法保证value不为null但是多加也没事
    ...
    public void dismissAndRemoveAllPopupWindow() {
        ...
        while (iterator.hasNext()) {
            ...
            if (popupWindow != null) {//add方法保证value不为null但是多加也没事
                popupWindow.dismiss();
            }
        }
    ...
    protected void onDestroy() {
        if (cancelRequest) {
            ...
            if (cancelType == CANCELREQUESTWITHTAG) {
                BeyondPhysicsManager.getInstance(getBeyondPhysicsManagerParams()).cancelRequestWithTag(activityKey, true);
            } else if (cancelType == CANCELREQUESTWITHTAGBYWAIT) {
                BeyondPhysicsManager.getInstance(getBeyondPhysicsManagerParams()).cancelRequestWithTagByWait(activityKey, true);//销毁前先取消网络请求,最后在ActivityManager移除baseActivity可以让代码更清晰
            }
        }
